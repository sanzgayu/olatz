...
void Simulation::decodage_ligne(string line) {   
	...
	enum Etat_lecture {NBP,PARTICULE,SPATIAL,REPARATEUR,NEUTRALISEUR,FIN};
...
for (size_t i(0);i< vec_particules.size(); i++){
			...
			if (collision_carre_cercle(c,c_spatial,false)) {
				cout << message::particle_robot_superposition(c.centre.x,c.centre.y,c.cote,
                                                  c_spatial.centre.x,c_spatial.centre.y,c_spatial.rayon);
				exit(EXIT_FAILURE); }
}
//collision repa//
for(size_t i(0);i< vec_particules.size(); i++){
	for(size_t j(0);j<vec_robots[1].size();j++) {
			...
			if (collision_carre_cercle(c1,c2,false)) {
				cout << message::particle_robot_superposition(c1.centre.x,c1.centre.y,c1.cote,
                                                  c2.centre.x,c2.centre.y,c2.rayon);
				exit(EXIT_FAILURE); }
	}
}
//collision neutra//
